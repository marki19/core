...
fun MediaItem?.toSongEntity(): SongEntity? =
    if (this != null) {
        SongEntity(
            ...
            albumId = null,
            albumName = this.mediaMetadata.albumTitle?.toString()?.takeIf { it != "null" && it.isNotBlank() },
            artistId = null,
            artistName = listOf(this.mediaMetadata.artist?.toString()?.takeIf { it != "null" && it.isNotBlank() } ?: "Various Artists"),
            duration = "",
            durationSeconds = 0,
            isAvailable = true,
            isExplicit = false,
            likeStatus = "INDIFFERENT",
            thumbnails = this.mediaMetadata.artworkUri?.toString()?.takeIf { it != "null" && it.isNotBlank() },
            title = this.mediaMetadata.title?.toString()?.takeIf { it != "null" && it.isNotBlank() } ?: "",
            videoType = "",
            category = "",
            resultType = "",
            liked = false,
