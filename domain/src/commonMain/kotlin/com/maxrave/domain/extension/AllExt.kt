...
fun GenericMediaItem.toSongEntity(): SongEntity =
    SongEntity(
        ...
        albumId = null,
        albumName = this.metadata.albumTitle?.takeIf { it != "null" && it.isNotBlank() },
        artistId = null,
        artistName = listOf(this.metadata.artist?.takeIf { it != "null" && it.isNotBlank() } ?: "Various Artists"),
        duration = "",
        durationSeconds = 0,
        isAvailable = true,
        isExplicit = false,
        likeStatus = "INDIFFERENT",
        thumbnails = this.metadata.artworkUri?.takeIf { it != "null" && it.isNotBlank() },
        title = this.metadata.title?.takeIf { it != "null" && it.isNotBlank() } ?: "",
        videoType = "",
        category = "",
        resultType = "",
        liked = false,
